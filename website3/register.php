...
if(isset($_POST['submit'])) {
 ...
 if ($row['count']==0) { 
  $query = $conn->prepare("INSERT INTO users3500(userName,userEmail,userPass,addressline1,addressline2,country,zipcode,city, admin) VALUES(?,?,?,?,?,?,?,?,?)");
  $query->bind_param('ssssssssi', $uname, $email, $hashed_password, $addressline1, $addressline2, $country, $zipcode, $city, $admin);  
  if ($query->execute()) {
						...
						<fieldset class="col-12">
							...
							<div class="form-group">
								<label style="font-size: 46px;">
									<input type="checkbox" id="buildingmanager" value = 1 name = 'admin' style="width:30px;height:30px;"> I am the building manager
								</label>
							</div>
